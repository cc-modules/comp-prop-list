...
const CompPropList = cc.Class({
  ...
  pluralize(type, array, compOrNode, prefixer = identity) {
    ...
    if (CC_DEBUG) {
      injected.forEach(newName => {
        ...
        cc.log(`${TAG} ${compOrNode.name}.${newName} = `, val);
  ...
  _saveFile() {
    saveFile(this.filename, this.__injected);
  }
});
...
function saveFile(filename, injected) {
  const path = Editor.url(`db://assets/scripts/${filename}`);
  const fs = require('fs');
  let code = [
    `export default function MainPropList<T = cc.Node> (Base) {`,
